...
class LoopingAnalysis(BaseAnalysis):
    ...
    def end_execution(self) -> None:
        for key, value in self.for_loop_count.items():
            logging.info('\nExecuted condition \t{}\t in file \t{}\t line number {} \t{}\t times'.format(key[3], key[1], key[2], value))
    ...
    def enter_for(self, dyn_ast: str, iid: int, next_value: Any, iterable: Iterable) -> Optional[Any]:
        ...
        target = node.target
        iter = node.iter
        target_code = cst.Module([target]).code
        iter_code = cst.Module([iter]).code
        #node_code.replace(body_code, "")

        loop_string = 'for {} in {}'.format(target_code, iter_code)
